...
def main_worker():
    ...
    # model = MIANet(args=args, layers=args.layers, classes=2, zoom_factor=8, \
    ...
    model = mymiaNet(args=args, layers=args.layers, classes=2, zoom_factor=8, \
                   criterion=nn.CrossEntropyLoss(ignore_index=255), BatchNorm=BatchNorm, \
    ...
    optimizer = torch.optim.SGD(
        [
            ...
            {'params': model.down_features.parameters()},
            {'params': model.LFG.parameters()},
            {'params': model.GIG.parameters()},
            ...
            {'params': model.gama_conv1.parameters()},
            {'params': model.gama_conv2.parameters()},
            {'params': model.gama_conv3.parameters()}
        ],
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (input, target, _, s_input, s_mask, subcls, class_chosen) in enumerate(tqdm(train_loader)):
        ...
        # torch.Size([4, 1, 3, 473, 473]) torch.Size([4, 1, 473, 473]) torch.Size([4, 3, 473, 473]) torch.Size([4, 473, 473]) torch.Size([4])
        output, main_loss, aux_loss, triplet_loss = model(s_x=s_input, s_y=s_mask, x=input, y=target,
                                                     class_chosen=class_chosen)

        if not args.multiprocessing_distributed:
            main_loss, aux_loss, triplet_loss = torch.mean(main_loss), torch.mean(aux_loss), torch.mean(triplet_loss)
        loss = main_loss + args.aux_weight * aux_loss + triplet_loss
        optimizer.zero_grad()
        ...
        triplet_loss_meter.update(triplet_loss.item(), n)
        ...
        #                 'triplet_loss {triplet_loss_meter.val:.4f} '
        ...
        #                                                   triplet_loss_meter=triplet_loss_meter,
...
def validate(val_loader, model, criterion):
    ...
    total_time = 0
    for e in range(1):
        for i, (input, target, _, s_input, s_mask, subcls, ori_label, _, class_chosen) in enumerate(tqdm(val_loader)):
