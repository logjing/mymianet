...
from model import myhdmnet as HDMNet
...
def train(train_loader, val_loader, model, optimizer, epoch):
    ...
    for i, (input, target, target_b, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        if (amp):
            ...
            output, main_loss, aux_loss1, compare_loss, kd_loss = model(s_x=s_input, s_y=s_mask, x=input, y_m=target,
                                                                        y_b=target_b, cat_idx=subcls)
            loss = main_loss + args.aux_weight1 * aux_loss1 + args.aux_weight2 * compare_loss + kd_loss
        ...
        aux_loss_meter2.update(compare_loss, n)
        aux_loss_meter3.update(kd_loss.item(), n)
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'AuxLoss3 {aux_loss_meter3.val:.4f} '
                        ...
                        'Accuracy {accuracy:.4f}.'.format(epoch + 1, args.epochs, i + 1, len(train_loader),
                                                          ...
                                                          aux_loss_meter3=aux_loss_meter3,
...
def map_to_square(x):  # 将mask变成正方形
    H, W = x.shape[-2:]
    longerside = max(H, W)
