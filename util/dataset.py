...
class SemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, base_data_root=None, data_list=None, data_set=None, use_split_coco=False, \
        ...
        for line in f_str:
            img, mask = line.split(' ')
            # img.replace("../","../MIANet-main/")
            img = img.replace("../data","E:\ShangJinlong\dataset")
            mask = mask.replace("../data", "E:\ShangJinlong\dataset")
            self.data_list.append((img, mask.strip()))

        with open(fss_sub_class_file_list_path, 'r') as f:
            f_str = f.read()
            f_str.replace("../","E:\ShangJinlong\dataset")
    ...
    def __getitem__(self, index):
        ...
        for k in range(self.shot):
            ...
            while((support_image_path == image_path and support_label_path == label_path) or support_idx in support_idx_list):
                ...
                support_image_path = support_image_path.replace("../data", "E:\ShangJinlong\dataset")
                support_label_path = support_label_path.replace("../data", "E:\ShangJinlong\dataset")
