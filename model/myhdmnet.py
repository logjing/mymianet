...
class OneModel(nn.Module):
    def __init__(self, args, cls_type=None):
        ...
        # )
        self.query_merge = nn.Sequential(
            nn.Conv2d(512 + 2, 256, kernel_size=1, padding=0, bias=False),
            nn.ReLU(inplace=True),
        )
...
#             param.requires_grad = False
#         self.teacher_net = Clip.encode_text
#
#         self.student_net = nn.Sequential(
#             nn.BatchNorm1d(77),
#             nn.Linear(in_features=77, out_features=128),
...
#
#     def forward(self, x):
#         teacher_feat =  self.teacher_net(x)
#         student_feat = self.student_net(x.float())
